...
global classify_model, graph, nameEntity_model
dir_path = os.getcwd()
word_vector = {}
...
def api_message():
    ...
    predict_result = None
    with graph.as_default():
        predict_result = classify_model.predict(message_vec)
    ...
    if predict_result == 0:
        ...
        if RO != None:
            ...
            MENU = -1
            logging.debug("LOG CREATED")
            logging.info("res_id: {}".format(ID))
            logging.info("previous: {}".format(data["message"]))
            logging.info("message reply: {}".format(sending_message))
            logging.info("log id: {}".format(log_id))
            logging.info("menu id: {}".format(MENU))
            return jsonify(userID=data["userID"],previous_message=data["message"],message=sending_message,
                            sys_question="",res_topic=ID,menu_id=MENU,log_id=log_id,request_count=data["request_count"] + 1)
        else:
            RES_NAME = -1
            MENU = -1
            question_stack.append([data["userID"],data["message"],0])
        ...
        if len(temp) > 0:
            ...
            for i in key_list:
                if re.search(i,data["message"]):
                    ...
                    return jsonify(userID=data["userID"],previous_message=previous_message,message=sending_message,
...
if __name__ == '__main__':
    yaml_file = open(dir_path + '/NLP_model/classify.yaml', 'r')
    loaded_model_yaml = yaml_file.read()
    yaml_file.close()
    classify_model = model_from_yaml(loaded_model_yaml)
    classify_model.load_weights(dir_path + "/NLP_model/classify.h5")
